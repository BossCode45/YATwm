...
{
	uint16_t *desktopViewports = new uint16_t[cfg.numWS*2];
	...
	for(int i = 0; i < cfg.numWS; i++)
	{
		for(int j = 0; j < cfg.workspaces[i].screenPreferencesc; j++)
		{
			...
			{
				desktopViewports[i*2] = (uint16_t)screens[cfg.workspaces[i].screenPreferences[j]].x;
				desktopViewports[i*2 + 1] = (uint16_t)screens[cfg.workspaces[i].screenPreferences[j]].y;
				break;
			}
		}
	}
	...
	XChangeProperty(globals.dpy, globals.root, netDesktopViewport, XA_CARDINAL, 16, PropModeReplace, (unsigned char*)desktopViewports, cfg.numWS*2);
